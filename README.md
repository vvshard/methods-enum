...
Design patterns `state` etc. dynamic polymorphism are often solved with dyn Trait objects.
...
___
# impl_match! macro
This is an item-like macro that wraps a state `enum` declaration and one or more impl blocks, allowing you to write `match` expressions without `match-arm`s in the method bodies of these `impl`, writing the `match-arm`s into the corresponding enum variants.
...
```
All the macro does is to complete the unfinished `match` expressions in the method bodies marked with `~`, with all `enum` variants with `=>` appended, and insert from the enum declaration of the corresponding `match-arm`s blocks.  
Instead of the methods omitted in the `enum` declaration, in resulting `match` will be: `(EnumName)::(Variant) => {}`.  
...
If you are using `enum` with fields, before the name of the method that uses them, specify the template for decomposing fields into variables. Example:
```rust
methods_enum::impl_match! {
...
enum Shape {
    Circle(f64): (radius)
        ...
        fmt(f)      { write!(f, "Circle(R: {radius:.1})") }
    ,
...
When required, you can [change the decomposition template]() before the next method name, including, for example, an empty `(..)` / `{..}`, so as not to put _-prefixes on unused variables.
...
- Code examples with `impl_match!` - [here]().
___
# gen() macro
